...
import "@/components/mac-os-terminal.css"
...
export function MacOsTerminal({
  ...
  return (
    <div className={cn("hmot-wrapper", className)} {...props}>
      <div className="hmot-terminal">
        {/* Title bar */}
        <div className="hmot-titlebar">
          <div className="hmot-dots">
            <span className="hmot-dot hmot-dot-close" />
            <span className="hmot-dot hmot-dot-minimize" />
            <span className="hmot-dot hmot-dot-maximize" />
          </div>
          <div className="hmot-title-group">
            {showIcon && (
              <svg
                className="hmot-title-icon"
                viewBox="0 0 16 16"
                fill="none"
            ...
            <span className="hmot-title">
              {title}
              {subtitle && ` \u2014 ${subtitle}`}
              {dimensions && ` \u2014 ${dimensions}`}
            </span>
          </div>
          <div className="hmot-dots-spacer" />
        </div>

        {/* Terminal body */}
        <div className="hmot-body" ref={bodyRef}>
          {steps.map((step, i) => (
            <div
              key={i}
              data-line
              className={cn(
                "hmot-line",
                step.type === "command" && "hmot-line-command",
                step.type === "blank" && "hmot-line-blank"
              )}
            >
              {step.type === "command" ? (
                <>
                  <span className="hmot-prompt">{prompt}</span>
                  <span data-typed className="hmot-typed" />
                </>
              ) : step.type === "prompt" ? (
                <>
                  <span className="hmot-text">{step.prefix}</span>
                  <span data-typed className="hmot-typed" />
                </>
              ) : step.type === "blank" ? (
                <br />
                ...
                <>
                  <span data-spinner className="hmot-spinner">
                    ⠋
                  </span>
                  <span className="hmot-text">{step.text}</span>
                </>
              ) : (
                <span
                  className={cn(
          ...
          <div ref={finalPromptRef} className="hmot-line">
            <span className="hmot-prompt">{prompt}</span>
            <span ref={cursorRef} className="hmot-cursor">
              ▋
            </span>
          </div>
        </div>
